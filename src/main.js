...
const app=$('#app');
app.innerHTML=`<header><a class="brand" href="./"><span class="brand-icon">C<span>✦</span></span><span>Ceo<span class="brand-light">-body</span><small>HUMAN ANATOMY STUDIO</small></span></a><div class="header-note" id="headerNote"></div><div class="header-actions"><button id="resultsButton"></button><button id="quality"></button><div class="language"><button id="th">ไทย</button><button id="en">EN</button></div></div></header>
...
function visible(p){
 ...
 if(currentLesson()){
  ...
  return p.system===lesson.system||active.has(p.id);
 }
 if(openedParent){return p.parent===openedParent;}
 if(p.parent)return false;
 if(state.isolated)return p.id===state.selected;
 if(state.system!=='all')return p.system===state.system;
...
function renderList(){
 ...
 let prev='';
 for(const p of list){
 if(p.system!==prev){const h=document.createElement('div');h.className='group-label';h.textContent=systems[p.system][state.lang==='th'?0:1];$('#partList').append(h);prev=p.system;}
...
function renderDetail(){
 const p=selected();$('#organName').textContent=title(p);$('#organAlt').textContent=state.lang==='th'?p.en:p.th;
 $('#organSystem').textContent=systems[p.system][state.lang==='th'?0:1];$('#organSystem').style.color=systems[p.system][2];
 ...
 $('#organFunction').textContent=p.desc[state.lang==='th'?0:1];
...
function quizBest(id){try{return Number(localStorage.getItem('ceo-body-quiz-'+id)||0);}catch{return 0;}}
function quizHistory(id){try{return JSON.parse(localStorage.getItem('ceo-body-quiz-history-'+id)||'[]');}catch{return [];}}
function saveQuizBest(id,score){try{if(score>quizBest(id))localStorage.setItem('ceo-body-quiz-'+id,String(score));const history=quizHistory(id);history.unshift({score,total:getQuiz(id).length,at:new Date().toISOString()});localStorage.setItem('ceo-body-quiz-history-'+id,JSON.stringify(history.slice(0,10)));}catch{}}
function renderResults(){
 const lang=state.lang==='th'?0:1,rows=lessonOrder.map(id=>{const lesson=getLesson(id),history=quizHistory(id),best=quizBest(id),last=history[0];return{id,title:lesson.title[lang],best,last,attempts:history.length,total:getQuiz(id).length};});
 const attempted=rows.filter(r=>r.attempts),sumBest=rows.reduce((s,r)=>s+r.best,0),sumTotal=rows.reduce((s,r)=>s+r.total,0);
 $('#resultsTitle').textContent=tr('ผลการเรียนและคะแนนแบบทดสอบ','Learning & quiz results');
 $('#resultsSummary').textContent=tr('ทำแบบทดสอบแล้ว '+attempted.length+' / '+rows.length+' ระบบ · คะแนนดีที่สุดรวม '+sumBest+' / '+sumTotal,'Completed '+attempted.length+' / '+rows.length+' quizzes · Combined best '+sumBest+' / '+sumTotal);
 $('#resultsTable').replaceChildren(...rows.map(r=>{const row=document.createElement('div');row.className='result-row';const last=r.last?new Date(r.last.at).toLocaleString(state.lang==='th'?'th-TH':'en-US',{dateStyle:'short',timeStyle:'short'}):'—';row.innerHTML='<div class="result-name"></div><div class="result-stat"></div><div class="result-stat"></div><div class="result-date"></div>';row.children[0].textContent=r.title;row.children[1].textContent=tr('ดีที่สุด '+r.best+'/'+r.total,'Best '+r.best+'/'+r.total);row.children[2].textContent=tr('ทำ '+r.attempts+' ครั้ง','Attempts '+r.attempts);row.children[3].textContent=last;return row;}));
 $('#resultsReset').textContent=tr('ล้างประวัติคะแนนในเครื่องนี้','Clear scores on this device');$('#resultsClose').title=tr('ปิดผลคะแนน','Close results');
}
...
function startQuiz(){const lesson=currentLesson();if(!lesson)return;stopPresentation();const questions=getQuiz(state.lesson);if(!questions.length)return;state.quiz={lessonId:state.lesson,index:0,answers:Array(questions.length).fill(null),score:0,answered:false,finished:false};renderQuiz();}
function closeQuiz(){state.quiz=null;$('#quizPanel').hidden=true;}
function renderQuiz(){
 const qstate=state.quiz,panel=$('#quizPanel');if(!qstate){panel.hidden=true;return;}const questions=getQuiz(qstate.lessonId),lesson=getLesson(qstate.lessonId),i=qstate.index,lang=state.lang==='th'?0:1;panel.hidden=false;
 $('#quizTitle').textContent=lesson.title[lang]+' · '+tr('แบบทดสอบ','Quiz');$('#quizClose').title=tr('ปิดแบบทดสอบ','Close quiz');
 if(qstate.finished){const best=quizBest(qstate.lessonId);$('#quizProgress').textContent=tr('สรุปคะแนน','Score summary');$('#quizQuestion').textContent=tr('ได้ '+qstate.score+' / '+questions.length+' คะแนน','Score: '+qstate.score+' / '+questions.length);$('#quizOptions').replaceChildren();$('#quizFeedback').textContent=tr('คะแนนดีที่สุดในเครื่องนี้: '+best+' / '+questions.length,'Best score on this device: '+best+' / '+questions.length);$('#quizNext').textContent=tr('ทำใหม่','Retry');return;}
...
function answerQuiz(choice){const qstate=state.quiz;if(!qstate||qstate.answered)return;const q=getQuiz(qstate.lessonId)[qstate.index];qstate.answers[qstate.index]=choice;qstate.answered=true;if(choice===q.answer)qstate.score++;renderQuiz();}
function nextQuiz(){const qstate=state.quiz;if(!qstate)return;const questions=getQuiz(qstate.lessonId);if(qstate.finished){qstate.index=0;qstate.answers=Array(questions.length).fill(null);qstate.score=0;qstate.answered=false;qstate.finished=false;renderQuiz();return;}if(!qstate.answered)return;if(qstate.index===questions.length-1){qstate.finished=true;saveQuizBest(qstate.lessonId,qstate.score);renderQuiz();return;}qstate.index++;qstate.answered=false;renderQuiz();}
function renderSimulation(){
 ...
 const info=lesson?lesson.title[state.lang==='th'?0:1]+' · '+currentLessonStep().body[state.lang==='th'?0:1]:tr('หัวใจบีบตัว • จุดแดง/น้ำเงินแสดงการไหลของเลือด • ปอดและกะบังลมแสดงการหายใจ • จุดสีเหลืองแสดงอาหาร','Heart contraction • red/blue particles show blood flow • lungs and diaphragm show breathing • yellow particles show food movement.');
 $('#simulationInfo').textContent=info;
}
function captureLessonReturn(){
 if(state.lessonReturn)return;state.lessonReturn={system:state.system,selected:state.selected,sex:state.sex,playing:state.playing,heart:state.heart,blood:state.blood,lungs:state.lungs,gut:state.gut,mode:state.mode,explode:state.explode,labels:state.labels,query:state.query,hidden:[...state.hidden],isolated:state.isolated,skin:$('#skin').checked,bones:$('#bones').checked,muscles:$('#muscles').checked,openedParent,cutaway};
}
function restoreLessonReturn(full=true){
 const r=state.lessonReturn;state.lessonReturn=null;if(!r)return;
 Object.assign(state,{sex:r.sex,playing:r.playing,heart:r.heart,blood:r.blood,lungs:r.lungs,gut:r.gut,mode:r.mode,explode:r.explode,labels:r.labels});
 ...
 $('#sex').value=state.sex;$('#explode').value=Math.round(state.explode*100);$('#explodeValue').value=Math.round(state.explode*100)+'%';$('#skin').checked=r.skin;$('#bones').checked=r.bones;$('#muscles').checked=r.muscles;for(const id of ['heart','blood','lungs','gut'])$('#'+id).checked=state[id];
}
...
function startLesson(id=state.lessonChoice){
 ...
 const sim=lesson.simulation||{};state.playing=true;state.heart=Boolean(sim.heart);state.blood=Boolean(sim.blood);state.lungs=Boolean(sim.lungs);state.gut=Boolean(sim.gut);
 for(const key of ['heart','blood','lungs','gut'])$('#'+key).checked=state[key];$('#skin').checked=false;$('#bones').checked=false;$('#muscles').checked=false;
...
function localize(){
 ...
 const texts={
 headerNote:['สื่อการสอนกายวิภาคแบบโต้ตอบ','Interactive anatomy for the classroom'],resultsButton:['ผลคะแนน','Results'],
 ...
 simulationLabel:['จำลองการทำงาน','Physiology'],heartText:['หัวใจ','Heart'],bloodText:['เลือด','Blood flow'],lungsText:['ปอด','Lungs'],gutText:['ลำไส้','Intestines'],
 detailEyebrow:['ชิ้นส่วนที่เลือก','SELECTED STRUCTURE'],functionHeading:['หน้าที่','Function'],layersHeading:['ชั้นและรูปแบบร่างกาย','Body layers & variant'],
...
$('#resultsButton').onclick=openResults;$('#resultsClose').onclick=closeResults;$('#resultsReset').onclick=resetResults;$('#resultsPanel').onclick=e=>{if(e.target===$('#resultsPanel'))closeResults();};
$('#search').oninput=e=>{state.query=e.target.value;renderList();};
$('#system').onchange=e=>{clearLessonState();state.system=e.target.value;state.isolated=false;openedParent=null;const p=parts.find(p=>allowed(p)&&(state.system==='all'||p.system===state.system));if(p)state.selected=p.id;update();resetCamera();};
...
function animate(now){
 ...
 const lessonSim=currentLesson()?.simulation||{};
 ...
 for(const {mesh,p,path,phase} of airDots){mesh.visible=Boolean(lessonSim.air)&&p.group.visible&&(!lessonStep||lessonStep.parts.includes(p.id));if(mesh.visible){const inhale=(Math.sin(elapsed*Math.PI*1.1)+1)/2,t=(elapsed*.12+phase)%1;mesh.position.copy(path.getPoint(inhale>.5?t:1-t));}}
 for(const {mesh,p,path,phase} of urineDots){mesh.visible=Boolean(lessonSim.urine)&&p.group.visible&&(!lessonStep||lessonStep.parts.includes(p.id));if(mesh.visible)mesh.position.copy(path.getPoint((elapsed*.055+phase)%1));}
 for(const {mesh,p,path,phase} of signalDots){mesh.visible=Boolean(lessonSim.signal)&&p.group.visible&&(!lessonStep||lessonStep.parts.includes(p.id));if(mesh.visible)mesh.position.copy(path.getPoint((elapsed*.22+phase)%1));}
...
$('#loading').remove();localize();resize();resetCamera();requestAnimationFrame(animate);
